...
{
    /// <summary>
    /// Bidirectional lookup between human-readable key names used in a SendKeys chord
    ...
    internal static class VirtualKeyMap
    {
        ...
        public static int? Resolve(string name)
        {
            ...
            if (trimmed.StartsWith("0x", StringComparison.OrdinalIgnoreCase) &&
                int.TryParse(trimmed.Substring(2), NumberStyles.HexNumber, null, out var hex))
            {
                return hex;
            }
            if (int.TryParse(trimmed, out var num)) return num;

            return NameToVk.TryGetValue(trimmed, out var vk) ? vk : (int?)null;
        }
